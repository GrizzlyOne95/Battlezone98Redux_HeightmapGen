...
FILE_DESCRIPTION = "Battlezone Heightmap Generator"
INTERNAL_NAME = "BZHeightmapGenerator"
ORIGINAL_FILENAME = "BZHeightmapGenerator.exe"
...
def normalize_version(value: str) -> tuple[tuple[int, int, int, int], str]:
    ...
    groups = match.groups()
    parts = [int(groups[0])]
    parts.extend(int(part) if part is not None else 0 for part in groups[1:])
...
def main() -> None:
    ...
    parser.add_argument("--version", required=True, help="Release version, for example 1.0.2 or v1.0.2")
